...
cem_initiatives <- read_xlsx("~/CEM/CEM Matrix/2021 June Matrix.xlsx", "Initiatives") %>%
  pivot_longer(!(iso2:iso3), names_to = "initiative", values_to = "participation")

cem_initiatives %<>% 
  replace_na(list(participation = 0)) %>%
  mutate(participation = participation * rep(1:length(unique(cem_initiatives$initiative)), 
                                             length(unique(cem_initiatives$iso3))))

eu_initiatives <- tibble(iso3 = eu) %>% 
  full_join(cem_initiatives %>% 
              filter(iso2 == "EC") %>% 
              select(initiative, participation), by = character()) %>%
  mutate(participation_eu = participation + length(unique(cem_initiatives$initiative)) + 1) %>%
  select(-participation)

data("World")

cem_initiatives_full <-
  expand_grid(iso3 = World$iso_a3, 
              initiative = unique(cem_initiatives$initiative)) %>%
  left_join(cem_initiatives, by = c("iso3", "initiative")) %>%
  left_join(eu_initiatives, by = c("iso3", "initiative")) %>%
...
World_cem_initiatives <- World %>%
  left_join(cem_initiatives_full, by = c("iso_a3" = "iso3")) %>%
...
####Matrix Chart####
cem_initiatives <- cem_initiatives %>% rename(workstream = initiative)
cem_campaigns <- cem_campaigns %>% rename(workstream = campaign)
